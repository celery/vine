...
def test_signal_callback_kwargs():
    callback = Mock(name='callback')
...
def test_call_ignore_result_callback_kwarg():
    fun = Mock(name='fun')
    callback = Mock(name='callback')
...
def test_with_partial_args_and_args():
    m = Mock(name='m')
    p = promise(m, (1, 2, 3), {'foobar': 2})
    p(4, 5, bazbar=3)
...
def test_throw_no_exc():
    p = promise()
    with pytest.raises((TypeError, RuntimeError)):
        p.throw()


def test_throw_no_excinfo():
    p = promise()
    with pytest.raises(KeyError):
        p.throw(KeyError())


def test_throw_with_tb():
    p = promise()
...
def test_weak_reference_unbound():
    def f(x):
        return x ** 2
